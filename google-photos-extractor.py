...
def extract_sort_date_and_description(description: str) -> str:
    ...
    if match:
        ...
        return format_date(year, SEASON.get(season)), get_description_without_date(description, REGEXP_EXACT_SEASON)
...
with zipfile.ZipFile(zip_path, 'r') as zf:
    ...
    for file in target_files:
        ...
        if match:
            ...
            if original_file in sort_dates_and_descriptions:
    ...
    for file in target_files:
        if not file.endswith('.json') and not file.endswith('/') and file not in files_to_remove:
            ...
            sort_date, description = sort_dates_and_descriptions.get(file_path.as_posix(), (None, None))
